...
// One gate shared by every attorney-general feed. They publish everything the
// office does — antitrust, environmental, criminal — so the headline has to be
// about consumers being ripped off, and the deny list drops subjects that would
// be jarring on a consumer page even when they mention fraud.
const AG_REQUIRES = /\b(scams?|fraud|fraudulent|defraud\w*|deceptive|robocalls?|telemarketing|identity theft|ponzi|pyramid scheme|price goug\w*|imposters?|impersonat\w*|phishing|consumer alert|consumer warning|warns? consumers?|sweepstakes|lottery|debt relief|predatory)\b/i;
const AG_EXCLUDES = /\b(immigration|immigrant|alien|visa|deport\w*|voting|election|ballot|firearms?|narcotics?|drug traffick\w*|fentanyl|opioid|child|minor|sexual|abortion|terror\w*|human traffick\w*|assault|murder|homicide|kidnap\w*|merger|antitrust|emissions|pollution)\b/i;

function stateAGs(list) {
    return list.map(s => ({
        id: s.agency.toLowerCase().replace(/\W+/g, '-'),
        agency: s.agency,
        label: s.label,
        url: s.url,
        trusted: false,
        requires: AG_REQUIRES,
        excludes: AG_EXCLUDES,
    }));
}
...
const SOURCES = [
    ...
    ...stateAGs([
        // State attorneys general are the other half of consumer protection, and
        // they warn about things the FTC never gets to — storm-chaser contractors
        // after a hurricane, a gift-card scam working one metro area. Like DOJ,
        // their feeds are all press releases, so the same strict gating applies.
        { agency: 'NC DOJ', label: 'North Carolina Department of Justice', url: 'https://ncdoj.gov/feed/' },
        { agency: 'AZ AG',  label: 'Arizona Attorney General',            url: 'https://www.azag.gov/rss.xml' },
        { agency: 'PA AG',  label: 'Pennsylvania Attorney General',       url: 'https://www.attorneygeneral.gov/feed/' },
        { agency: 'CO AG',  label: 'Colorado Attorney General',           url: 'https://coag.gov/press-releases/feed/' },
    ]),
];
...
function isGovUrl(url) {
    try {
        const u = new URL(url);
        // Any hostname under .gov, rather than a hand-maintained list of agencies.
        // The registry restricts .gov to US government bodies, so this is the same
        // guarantee with none of the maintenance — the previous list silently
        // dropped every DOJ item because justice.gov had not been added to it,
        // and that failure is invisible: the feed just looks empty.
        return u.protocol === 'https:' && /(^|\.)gov$/.test(u.hostname);
