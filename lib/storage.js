...
// The store is PRIVATE, deliberately: these documents include newsletter
// subscribers and contact-form submissions — real people's email addresses. In a
// public store those sit at a fetchable URL, which is not somewhere personal
// data belongs. So reads go through the authenticated get(), not a plain fetch
// of a public URL.
async function blobRead(name, fallback) {
    const result = await blobApi.get(name + '.json', { access: 'private' });
    if (!result) return fallback;                    // never written yet
...
async function blobWrite(name, data) {
    ...
    await blobApi.put(name + '.json', JSON.stringify(data, null, 2), {
        access: 'private',
